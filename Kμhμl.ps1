...
function Show-ASXTuiWorkstation {
    ...
    $asxTuiPath = Join-Path $PSScriptRoot "asx-tui.ps1"
    if (-not (Test-Path $asxTuiPath)) {
        Write-Quantum "# Missing ASX T-UI script: $asxTuiPath" -Raw
        return
    }

    $ASXTuiWorkstation = Get-Content $asxTuiPath -Raw
